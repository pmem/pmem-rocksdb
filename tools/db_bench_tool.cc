...
DEFINE_bool(recycle_dcpmm_sst, false, "Recycle obsoleted sst to avoid page-fault and page-zeroing overhead on pmem");
...
DEFINE_bool(dcpmm_wal_enable, false,
             "Store WAL file to dcpmm device and use pmdk to write/read it. ");

DEFINE_bool(dcpmm_kvs_enable, false, "If separately store values with libpmemobj");
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    const int test_duration = write_mode == (RANDOM&&!fill) ? FLAGS_duration : 0;
