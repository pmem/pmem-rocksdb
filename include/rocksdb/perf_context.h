...
struct PerfContext {
  ...
  uint64_t total_get_time;

  uint64_t match_filter;
  uint64_t retrieve_data_block;
  uint64_t data_block_seek;
  uint64_t update_block_cache;
  ...
  // my write stats
  uint64_t write_time;
  uint64_t write_process_time;
  uint64_t writer_wait;
