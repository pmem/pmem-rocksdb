  ...
  if (block_cache != nullptr || block_cache_compressed != nullptr) {
    ...
    if (block_entry->GetValue() == nullptr && !no_io && ro.fill_cache) {
      ...
      if (!contents) {
        ...
        if ((rep_->table_options.cache_data_blocks_for_mmap_read &&
             block_type == BlockType::kData) ||
            (rep_->table_options.cache_index_and_filter_blocks_for_mmap_read &&
             (block_type == BlockType::kFilter ||
              block_type == BlockType::kIndex))) {
          contents->GetDataOwnership(
...
Status BlockBasedTable::RetrieveBlock(
  ...
  MemoryAllocator* allocator = GetMemoryAllocator(rep_->table_options);
  {
    ...
    s = ReadBlockFromFile(
        ...
        block_type == BlockType::kData
            ? rep_->table_options.read_amp_bytes_per_bit
            : 0,
        allocator, for_compaction, rep_->blocks_definitely_zstd_compressed,
        rep_->table_options.filter_policy.get());
  }
  ...
  if ((rep_->table_options.cache_data_blocks_for_mmap_read &&
       block_type == BlockType::kData) ||
      (rep_->table_options.cache_index_and_filter_blocks_for_mmap_read &&
       (block_type == BlockType::kFilter || block_type == BlockType::kIndex))) {
    block->GetDataOwnership(allocator);
  }
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  bool may_match;
  {
    PERF_TIMER_GUARD(match_filter);
    may_match =
        FullFilterKeyMayMatch(read_options, filter, key, no_io,
                              prefix_extractor, get_context, &lookup_context);
  }
  if (!may_match) {
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      {
        PERF_TIMER_GUARD(data_block_seek);
        may_exist = biter.SeekForGet(key);
      }
